...
| `apps/api/src/scraper/scrapeURL/engines/zapfetch-cache/**` | new files | `ZAPFETCH_USE_CACHE` | Cache engine implementation + storage singleton + write-back transformer. Lookup default TTL is configurable via `ZAPFETCH_CACHE_DEFAULT_MAX_AGE_MS` (`getDefaultMaxAge`); writeback TTL is hardcoded to fork default 24h (`getWritebackTtl`) so a "lookup-off, writeback-on" canary stays achievable when the lookup default is set to 0. | `docs/response-cache-plan.md` |
...
| `apps/api/src/config.ts` | cache section | n/a (this file defines the switch) | Adds `ZAPFETCH_USE_CACHE` + `CACHE_DATABASE_URL` + `CACHE_OSS_*` (incl. `CACHE_OSS_PREFIX`, default `docs/`) + `ZAPFETCH_CACHE_DEFAULT_MAX_AGE_MS` env vars. ZF-9 dev/prod isolation sets dev `CACHE_OSS_PREFIX=docs-dev/`. | — |
...
| `apps/api/src/scraper/scrapeURL/engines/zapfetch-cache/index.ts` | `sendDocumentToZapfetchCache` writeback gate | `ZAPFETCH_USE_CACHE` + `ZAPFETCH_CACHE_DEFAULT_MAX_AGE_MS` | Writeback path uses `getDefaultMaxAge()` directly instead of caller-supplied `maxAge`. Caller's `maxAge=0` is a read-side "force fresh fetch" signal — it should not also disable populating shared cache for future readers. The writeback kill-switch is now solely the fork-default TTL (set `ZAPFETCH_CACHE_DEFAULT_MAX_AGE_MS=0` to disable). | ZF-2 Phase-1 假灰度根因 followup |
